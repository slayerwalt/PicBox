...
const GetImages = (folderPath) => {
  ...
  axios
    ...
    .then((res: any) => {
      ...
      res.data.forEach((e) => {
        if (e.download_url) {
          e.origin_url = `https://raw.githubusercontent.com/${github_config.owner}/${github_config.repoPath}/main/${folderPath}/${e.name}`
      ...
      <div v-for="item in images" :key="`${item.name}`" class="item">
        ...
        <div class="info">
          ...
          <div class="copy-box">
            <span
              class="copy-btn"
              v-bind:data-clipboard-text="GetOriginText(item.origin_url)"
              @click="CopyText()"
              >url</span
            ><span
              class="copy-btn"
